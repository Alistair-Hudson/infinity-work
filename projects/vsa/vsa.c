...
/******INTERNAL FUNCTIONS******/
static block_t *DeFrag(block_t *block, block_t *prev_block);
static long ABSBlockSize(block_t *block);
...
vsa_t *VSAInit(void *buffer, size_t buffer_size)
{
	...
	first_block->size_of_block = (new_vsa->free_space - HEADER_SIZE);
	final_block = (block_t*)buffer + (buffer_size - HEADER_SIZE)/HEADER_SIZE;
	final_block->size_of_block = END;
...
void *VSAAlloc(vsa_t *allocator, size_t block_size)
{
	...
	while ((END != block_head->size_of_block) && 
	...
	if (END != block_head->size_of_block)
	{
		...
		if (0 > free_space)
		{
			free_space = block_head->size_of_block;
		}
