...
* Return : none.
 */
void ListSetData(iter_t *iterator, void *value); 

/*
* Input : iterator, void pointer to data ;
* Process :Insert address of data before iterator ;
* Return : new iterator if successful,  iterator to the end if fail.
...
*/
iter_t SListBegin(const list_t *);

/*
Input: pointer to List struct 
Return: iterator that points to the end in Linked List;
*/
iter_t SListEnd(const list_t *);
...
/*
 * Input: Two iterators;
 * Process: The function compares iterators.
 * Return: 
 * 1 -  in case is equal
 * 0 - if is not equal
 */
